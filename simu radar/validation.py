...
def calibration(L,cm,dimension,W,H):
    ...
    for i in range(len(cm)):
        L1 = np.sqrt(((value_copy[i,0]-cm1[:,0].tolist())**2+(value_copy[i,1]-cm1[:,1].tolist())**2)).tolist()
        index_i1 = L1.index(min(L1))
    ...
    return donnée
count = 250
name_data = 'data-validation'
W = 1280
H = 720
while(os.path.isfile('C:/Users/Gauthier_Rotsart/Documents/yolov5/data/images/'+name_data+'/image_'+str(count).zfill(4)+'.jpg') == 1):
